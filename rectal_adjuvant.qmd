---
title: "Rectal Cancer Adjuvant Therapy"
format: 
  revealjs:
    slide-number: c/t
    width: 1600
    height: 900
    logo: "atriumlogo.jpg"
    footer: "[GI Surgical Oncology Talks](https://gisurgonc.github.io/talks/)"
    #css: ["theme/theme.css"]
    theme: default
      
    echo: false
editor: visual
bibliography: zotero.bib
---

## Trans-abdominal Rectal Surgery

Trans-abdominal procedures include

- Low anterior resection: Removal of the rectum with an anastomosis between colon and distal rectum\
- Hartmann resection: Removal of the rectum with end colostomy. The rectal stump is stapled. The anal sphincters are left in situ\
- Abdominoperineal resection: Removal of rectum and anus from both an abdominal and perineal approach. The anal sphincters are removed.\

## GI Tumor Study Group 1985 Adjuvant Radiation

Surgery as sole treatment for rectal cancer $\rightarrow$ unacceptable rate of local recurrence.

\

GITSG Trial used postop radiation to reduce local recurrences

227 rectal cancer patients with curative surgical resection:

\

- 58: Surgery with no further therapy (Control Group)
- 50: Postop radiation alone 4000 or 4800cGy
- 48: Postop chemotherapy 5FU+methyl-CCNU
- 46: Chemo + radiation

::: aside
[@gastrointestinaltumorstudygroup1465]
:::

## GI Tumor Study Group 1985 Adjuvant Radiation

Local Recurrence

- 55%: Control with no further therapy
- 50%: Postop radiation alone 4000 or 4800cGy
- 48%: Postop chemotherapy 5FU+methyl-CCNU
- [33%: Chemo + radiation]{style="color: red"}

$\Rightarrow$ Reduction in local recurrence

No difference in overall survival

::: aside
[@gastrointestinaltumorstudygroup1465]
:::

## Overview of Rectal Adjunctive Therapy

![](images/rectal_cancer_rx_history_no_swedish.png)

## Total Mesorectal Excision (1986)

115 rectal cancer patients underwent surgery over 7.5 years

::::: {.columns align="center"}
::: {.column width="50%"}
69 with anastomosis below 5cm

Median followup of 4.2 years:

- 3 pelvic recurrences

- No staple-line recurrence
:::

::: {.column width="50%"}
![](images/tme_bordeianou.png)
:::
:::::

::: aside
[@heald1479]
:::

## Total Mesorectal Excision

![](images/mery_bleday.jpg)

::: aside
[@mery117]
:::

## Dutch TME + Postop ChemoRT Trial

Dutch trial used protocol-drived total mesorectal excision (TME)

\

Surgery alone

*vs*

Surgery $\rightarrow$ Chemoradiation

\

Postop ChemoRT reduced local recurrence from 10.9% to 5.6%

No difference in overall survival

## Dutch Preop Short-course Trial

Preoperative short-course radiation was designed to reduce time to surgery

1861 patients randomized:

- Preop RT (5x 500cGy) $\rightarrow$ Surgery (TME)
- Surgery alone (TME)

$\Rightarrow$ Reduction in local recurrence (2.4% vs 8.2%)

No difference in survival

Preoperative = *neoadjvant* therapy

::: aside
[@kapiteijn638]
:::

## Dutch Preop Short-course Trial Results

![](https://deidt7p41jzcy.cloudfront.net/AnnGastroenterol-35-226-g004.jpg)

## German Trial Preop vs Postop ChemoRT (AIO-94)

Preop radiation has theoretical advantage of improved efficacy

All patients received postoperative adjuvant chemotherapy

Preoperative ChemoRT was superior in:

- Local recurrence (6% preop vs 13% postop)
- toxicity (27% vs 40%)
- sphincter preservation (39% vs 19%)

No difference in overall survival or disease-free survival

NSABP R-03 Trial did not reach accrual goal

::: aside
[@sauer1731]
:::

## Overview of Rectal Adjunctive Therapy

![](images/rectal_cancer_rx_history_no_swedish.png)

## ECORT 22921 Postopo Chemotherapy

Postoperative (adjuvant) chemotherapy was shown to be effective in reducing distant metastasis and improving survival in the ECORT 22921 clinical trial.

\

Standard treatment now became:

\

Neoadjuvant ChemoRT $\rightarrow$ Surgery $\rightarrow$ Adjuvant Chemo

## Neoadjuvant Chemotherapy Strategy

Chemotherapy moved to *before* surgery with goals to:

- Improve tolerance of chemotherapy
- Reduce toxicity of chemotherapy
- Improve compliance with chemotherapy

$\Rightarrow$ Improve distant disease control

## PRODIGE-23 Postop vs Preop Chemo

PRODIGE 23 randomized 461 patients with T3 or T4 rectal cancers:

Long-course radiation $\rightarrow$ surgery $\rightarrow$ Chemo

Induction chemotherapy $\rightarrow$ Long-course radiation $\rightarrow$ surgery $\rightarrow$ Chemo

- Increased 3-year survival (76% vs 69%)

- Increased pathologic complete response (pCR) 28% vs 12%

Strictly speaking PRODIGE-23 is *not* TNT

::: aside
[@conroy702]
:::

## Total Neoadjuvant Therapy (TNT)

\
TNT approach moves *all* therapy to before surgery with goals to:

\

- Improve distant disease control ($\rightarrow$ Overall survival)
- Improve clinical complete response ($\rightarrow$ Organ preservation)

## RAPIDO - Preop short-course radiation $\rightarrow$ Preop Chemo

920 patients with T4 or node-positive disease:

Short-course RT $\rightarrow$ Chemo $\rightarrow$ Surgery\
vs\
ChemoRT $\rightarrow$ TME $\pm$ Chemo

\

pCR rate significantly higher in the RT $\rightarrow$ Chemo $\rightarrow$ Surgery (28% vs 14%)

Disease-specific survival at 3years was higher (30% vs 24%).

::: aside
[@bahadoer29]; [@vandervalk75]
:::

## RAPIDO vs PRODIGE-23

![](images/rapido_prodige23.jpg)

## Watchful Waiting for Rectal Organ Preservation

Retrospective studies: some patients with a complete clinical response to chemoradiation for rectal cancer can be managed *without* surgery.

\

Subsequent trials of neoadjuvant therapy included non-operative management as a treatment option in patients with a complete clinical response to neoadjuvant therapy.

## Watch and Wait - MSKCC

Restrospective comparison two groups with rectal cancer:

136 preop ChemoRT $\rightarrow$ surgery $\Rightarrow$ pCR

113 preop ChemoRT $\Rightarrow$ complete clinical response (cCR)

::: aside
[@smithe185896]
:::

## Watch and Wait - MSKCC

Restrospective comparison two groups with rectal cancer:

136 preop ChemoRT $\rightarrow$ surgery $\Rightarrow$ pCR

- No pelvic recurrences

113 preop ChemoRT $\Rightarrow$ complete clinical response (cCR)

- 22 local regrowth on surveillance $\rightarrow$ Surgery
  - Pelvic control mantained in 20 (91%)
- Rectal preservation in 93 (82%)

::: aside
[@smithe185896]
:::

## Watch and Wait - MSKCC

Restrospective comparison two groups with rectal cancer:

136 preop ChemoRT $\rightarrow$ surgery $\Rightarrow$ pCR

- 94% survival at 5 years

113 preop ChemoRT $\Rightarrow$ complete clinical response (cCR)

- 73% survival at 5 years
- [Higher rate of distant metastasis in those who had tumor regrowth (36% vs 1%)]{style="color:red"}

::: aside
[@smithe185896]
:::

## Total Neoadjuvant Therapy

TNT approach moves *all* therapy to before surgery with goals to:

- Improve distant disease control
- Improve clinical complete response

$\Rightarrow$ Facilitate organ preservation AND Improve overall survival

## OPRA Trial - Organ Preservation in Rectal Adenocarcioma

324 rectal cancer patients randomized

Chemo $\rightarrow$ ChemoRT $\rightarrow$ Restaging

ChemoRT $\rightarrow$ Chemo $\rightarrow$ Restaging

Restaging:

- Clinical response offered watch/wait

- Non-responders $\rightarrow$ surgery

::: aside
[@garcia-aguilar2546]
:::

## OPRA Trial - Organ Preservation in Rectal Adenocarcioma

ChemoRT: 5000 to 5600 cGy with 5-FU or Capcitabine

Chemo: FOLFOX or CapeOx

::: aside
[@garcia-aguilar2546]
:::

## OPRA Trial - Organ Preservation in Rectal Adenocarcioma

Restaging at 8 $\pm$ 4 weeks

- MRI pelvis
- CT chest/abdomen/pelvis
- Digital rectal exam
- Endoscopy

Categorized:

- Incomplete response $\rightarrow$
- Near complete response $\rightarrow$ Watch/Wait
- Complete response $\rightarrow$ Watch/Wait

::: aside
[@garcia-aguilar2546]
:::

## OPRA Trial - Organ Preservation in Rectal Adenocarcioma

304 patients restaged: Surgery recommended in 26%

225 patients Watch and Wait

Chemo $\rightarrow$ ChemoRT 40% recurrence

ChemoRT $\rightarrow$ Chemo 27% recurrence

::: aside
[@garcia-aguilar2546]
:::

## OPRA Trial - Organ Preservation in Rectal Adenocarcioma

\

225 patients Watch and Wait

Chemo $\rightarrow$ ChemoRT 39% organ preservation

ChemoRT $\rightarrow$ Chemo 54% organ preservation

::: aside
[@garcia-aguilar2546]
:::

## Watch/Wait Surveillance - OPRA

- DRE/Endoscopy q4 months x2 years $\rightarrow$ q6 months x3 years
- Rectal MRI q6 months x2 years $\rightarrow$ annually x3 years

::: aside
[@smith767]
:::

## Complete Clinical Response

Flexible sigmoidoscopy:

- pale smooth scar with or without telangiectasia
- no ulceration, nodularity, mucosal irregularities, or stricture
- digital rectal examination: smooth, flat scar without nodularity

Diffusion-weighted MRI:

- fibrotic, linear scar with low signal intensity on T2-weighted images
- no diffusion restriction
- no suspicious lymph nodes.

::: aside
NCCN Guidelines
:::

## Near Complete Clinical Response

Flexible sigmoidoscopy:

- Irregular small mucosal nodules, superficial ulceration, or mild persistent erythema

Digital Rectal Exam:

- Smooth induration or superficial minor mucosal irregularity

Diffusion-weighted MRI:

- smooth induration or superficial minor mucosal irregularity

T2-weighted MRI:

- downstaging with or without residual fibrosis and small area of residual signal

::: aside
NCCN Guidelines
:::

## Complete vs Near Complete clinical response

::: aside
[@smith767]
:::

## Chemo Intensification with TNT - FOLFIRINOX vs FOLFOX

![](images/janus_trial.png)

## Swedish Short-course RT {#Swedish}

Swedish Rectal Cancer Trial compared surgery alone with preoperative short-course therapy consisting of 5 doses of 500cGy of radiation without chemotherapy administered in one week prior to surgery.

Local recurrence was 9% in the therapy group vs 26% in the control group

Overall survival at 5 years of 58% vs 48% (p=0.004)

Disease-specific survival at 9 years 74% vs 65%. (p=0.002)

Of note, this trial was performed in the era prior to the widespread use of total mesorectal excision.

::: aside
[@swedishrectalcancertrial980]
:::

## MSI High Rectal Cancer

Mismatch repair protein deficient (MSI-high) colorectal cancer (due to either Lynch Syndrome or BRAF-1 mutation) responds poorly to chemotherapy but responds well to immunotherapy with PD-L1 blockade

See [CRC Genetics](crs_genetics_slides.qmd)

## Dostarlimab for Rectal Cancer - MSKCC

12 patients with MMR-deficient (MSI-high) rectal cancer

Treatment with PD-L1 blockade with 6 months of dostarlimab for 12 months

100% clinical response rate 

No patients were subsequently treated with either chemoradiation or surgery as originally planned.

::: aside
[@cercek2363]
:::



## Swedish Rectal Cancer Trial

![](https://deidt7p41jzcy.cloudfront.net/AnnGastroenterol-35-226-g003.jpg)

## Stockholm III

840 patients with rectal cancer randomized to preoperative radiation:

- Short course (5 x 5Gy) with surgery the following week
- Short course (5 x 5 Gy) with surgery 4- weeks later
- Long-course (25 x 2Gy) with surgery 4-8 weeks later

With short-course radiation, delayed surgery resulted in higher rates of pCR (11.8% vs 1.7%)

::: [aside]
[@pettersson972]
:::

## Stockholm III

![](https://deidt7p41jzcy.cloudfront.net/AnnGastroenterol-35-226-g006.jpg)

## References
